...
export function SubjectManagement({ course, onBack }: SubjectManagementProps) {
  ...
  const handleFinishSubject = async (subjectId: number) => {
    if (!confirm('Finish this subject? All in-progress tasks will be marked as finished.')) {
      return;
    }
    try {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'IN_PROGRESS': return { class: 'bg-blue-100 text-blue-700', label: 'In Progress' };
      case 'FINISHED': return { class: 'bg-green-100 text-green-700', label: 'Finished' };
      default: return { class: 'bg-gray-100 text-gray-700', label: 'Not Started' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-lg font-semibold">Subjects & Tasks</h4>
        <button
          ...
          return (
            <div key={subject.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6">
                ...
                <p className="text-gray-600 text-sm mb-4">{subject.description || 'No description'}</p>

                <div className="border-t pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-gray-700 font-medium">Tasks ({subject.tasks?.length || 0})</span>
                    <button
                      onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h4 className="text-lg font-semibold">Create New Subject</h4>
